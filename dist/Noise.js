function openSimplexNoise(clientSeed) {
    "use strict";
    var SQ5 = 2.23606797749979;
    var SQ4 = 2;
    var SQ3 = 1.7320508075688772;
    var toNums = function (s) { return s.split(",").map(function (s) { return new Uint8Array(s.split("").map(function (v) { return Number(v); })); }); };
    var decode = function (m, r, s) { return new Int8Array(s.split("").map(function (v) { return parseInt(v, r) + m; })); };
    var toNumsB32 = function (s) { return s.split(",").map(function (s) { return parseInt(s, 32); }); };
    var NORM_2D = 1.0 / 47.0;
    var NORM_3D = 1.0 / 103.0;
    var NORM_4D = 1.0 / 30.0;
    var SQUISH_2D = (SQ3 - 1) / 2;
    var SQUISH_3D = (SQ4 - 1) / 3;
    var SQUISH_4D = (SQ5 - 1) / 4;
    var STRETCH_2D = (1 / SQ3 - 1) / 2;
    var STRETCH_3D = (1 / SQ4 - 1) / 3;
    var STRETCH_4D = (1 / SQ5 - 1) / 4;
    ...
    var gradients2D = decode(-5, 11, "a77a073aa3700330");
    var gradients3D = decode(-11, 23, "0ff7mf7fmmfffmfffm07f70f77mm7ff0ff7m0f77m77f0mf7fm7ff0077707770m77f07f70");
    var gradients4D = decode(-3, 7, "");
    var lookupPairs2D = function () { return new Uint8Array([0, 1, 1, 0, 4, 1, 17, 0, 20, 2, 21, 2, 22, 5, 23, 5, 26, 4, 39, 3, 42, 4, 43, 3]); };
    ...
    var setOf = function (count, cb) {
        if (cb === void 0) { cb = function (i) { return i; }; }
        var a = [], i = 0;
        while (i < count) {
            a.push(cb(i++));
        }
    ...
    var doFor = function (count, cb) { var i = 0; while (i < count && cb(i++) !== true)
        ; };
    ...
    function createContribution(type, baseSet, index) {
        var i = 0;
        var multiplier = baseSet[index++];
        var c = { next: undefined };
    ...
    function createContributionArray(type) {
        var conts = [];
        var d = type.dimensions;
        var baseStep = d * d;
        var k, i = 0;
        while (i < type.pD.length) {
            var baseSet = type.base[type.pD[i]];
            var previous = void 0, current = void 0;
            k = 0;
            do {
        ...
        var result = [conts, createLookupPairs(type.lookup, conts)];
        type.base = undefined;
        type.lookup = undefined;
        return result;
    }
    var _a = createContributionArray(types._2D), contributions2D = _a[0], lookup2D = _a[1];
    var _b = createContributionArray(types._3D), contributions3D = _b[0], lookup3D = _b[1];
    var _c = createContributionArray(types._4D), contributions4D = _c[0], lookup4D = _c[1];
    var perm = new Uint8Array(256);
    var perm2D = new Uint8Array(256);
    var perm3D = new Uint8Array(256);
    var perm4D = new Uint8Array(256);
    var source = new Uint8Array(setOf(256, function (i) { return i; }));
    ...
    var API = {
        noise2D: function (x, y) {
            ...
            var xs = x + stretchOffset, ys = y + stretchOffset;
            var xsb = Math.floor(xs), ysb = Math.floor(ys);
            var squishOffset = (xsb + ysb) * SQUISH_2D;
            var dx0 = x - (xsb + squishOffset), dy0 = y - (ysb + squishOffset);
            var c = (function () {
                var xins = xs - xsb, yins = ys - ysb;
                var inSum = xins + yins;
                return lookup2D.get((xins - yins + 1) |
                    (inSum << 1) |
                    ((inSum + yins) << 2) |
                    ((inSum + xins) << 4));
            })();
        ...
        noise3D: function (x, y, z) {
            ...
            var xs = x + stretchOffset, ys = y + stretchOffset, zs = z + stretchOffset;
            var xsb = Math.floor(xs), ysb = Math.floor(ys), zsb = Math.floor(zs);
            var squishOffset = (xsb + ysb + zsb) * SQUISH_3D;
            var dx0 = x - (xsb + squishOffset), dy0 = y - (ysb + squishOffset), dz0 = z - (zsb + squishOffset);
            var c = (function () {
                var xins = xs - xsb, yins = ys - ysb, zins = zs - zsb;
                var inSum = xins + yins + zins;
                return lookup3D.get((yins - zins + 1) |
                    ((xins - yins + 1) << 1) |
                    ((xins - zins + 1) << 2) |
                    (inSum << 3) |
                    ((inSum + zins) << 5) |
            ...
            var i, value = 0;
            while (c !== undefined) {
                var dx = dx0 + c.dx, dy = dy0 + c.dy, dz = dz0 + c.dz;
        ...
        noise4D: function (x, y, z, w) {
            ...
            var xs = x + stretchOffset, ys = y + stretchOffset, zs = z + stretchOffset, ws = w + stretchOffset;
            var xsb = Math.floor(xs), ysb = Math.floor(ys), zsb = Math.floor(zs), wsb = Math.floor(ws);
            var squishOffset = (xsb + ysb + zsb + wsb) * SQUISH_4D;
            var dx0 = x - (xsb + squishOffset), dy0 = y - (ysb + squishOffset), dz0 = z - (zsb + squishOffset), dw0 = w - (wsb + squishOffset);
            var c = (function () {
                var xins = xs - xsb, yins = ys - ysb, zins = zs - zsb, wins = ws - wsb;
                var inSum = xins + yins + zins + wins;
                return lookup4D.get((zins - wins + 1) |
                    ((yins - zins + 1) << 1) |
                    ((yins - wins + 1) << 2) |
                    ((xins - yins + 1) << 3) |
                    ((xins - zins + 1) << 4) |
            ...
            var i, value = 0;
            while (c !== undefined) {
                var dx = dx0 + c.dx, dy = dy0 + c.dy, dz = dz0 + c.dz, dw = dw0 + c.dw;
